...
- `python` と `full` では、`python3`・`pip` と Python ツールは `/opt/pytools` の venv (`claude` ユーザー所有) から PATH に載ります。セットアップスクリプトやターミナルの `pip install` は sudo なしでそこに入り、OS の Python (`/usr/bin/python3`) はそのままです。
