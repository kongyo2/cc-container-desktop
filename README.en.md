...
2. On the **Images** page pick the variant you need and press **Download and register**. The image is pulled and recorded; the card turns "Registered". Web is the recommended first pick. An image someone else published, or one you built yourself, goes in through "Custom image" on the same page.
...
Every image shares one foundation (Ubuntu 24.04, Node.js 24, Claude Code, git, GitHub CLI, tmux, ripgrep, fd, jq, yq, gcc/make), so the app behaves the same on all of them. Pick one of eight variants by what your repository needs.
...
- The app does not pull by tag. Its bundled catalog (`src/shared/imageCatalog.json`) pins the platform manifest digest of every variant, and the registration it keeps on disk stores that digest, so a moved tag never changes what a task runs. A release whose digest is not in the catalog yet is pulled by tag, and the digest that actually arrives is pinned at registration (an image that merely sits locally under that tag without ever coming from a registry has no digest, so it is followed by its tag).
- A pulled image is registered as it is; nothing inside it is inspected. "Custom image" takes any reference `docker pull` takes (`ghcr.io/owner/image:tag`, `owner/image@sha256:…`, a locally built `my-image:dev`) and fetches it exactly as `docker pull` would. A reference with `@sha256:` is pinned to that digest; a tag is followed (pull or rebuild it and its tasks show "image change ready to apply", applied with "Recreate").
- In `python` and `full`, `python3`, `pip` and the Python tools on PATH come from the venv at `/opt/pytools`, which the `claude` user owns: `pip install` in a setup script or a terminal works without `sudo` and installs there, and the OS Python (`/usr/bin/python3`) is left alone.
- To run as a task an image needs the `claude` user (uid/gid 1000), `/home/claude/workspace`, and `node`, `claude`, `tmux`, `bash` and `git` on the PATH. Base it on the images under `docker/`, or lay it out the same way.
...
Anonymous pulls from Docker Hub are rate limited. While limited, run `docker login` in a terminal, run the digest-pinned pull command from the card's "Details", then press the same button in the app: it registers the image that is now local. The app never reads your credentials.
...
npm run check            # format / lint / knip / typecheck / unit tests / catalog / build
...
- Every image carries `/opt/cc/image-info.json` (variant, release, source revision, measured tool versions) and `/opt/cc/apt-packages.txt`. The tool versions in the catalog are read from it.
- Publishing is the `publish-images` GitHub Actions workflow (workflow_dispatch). It builds variant × platform on native runners, pushes by digest, merges each variant into a candidate tag, pulls the candidates back from Docker Hub and reads their image-info, promotes the indexes to the official tags, confirms anonymous pulls, then generates `imageCatalog.json` with the real digests and sizes and opens a pull request. Secrets: `DOCKERHUB_USERNAME` / `DOCKERHUB_TOKEN`; the target comes from `DOCKERHUB_REPOSITORY` (default `docker.io/kongyo2/cc-workbench`), read from the repository variables or, failing that, the repository secrets — a variable is preferable, since a secret reads as `***` in the logs. It must be lowercase, host-qualified and untagged (`docker.io/namespace/name`), or the first job stops the run.
