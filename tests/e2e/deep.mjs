...
try {
  ...
  const baseImage = await session.registerImage();
  const defaultEnvironmentId = await session.ensureEnvironment({ name: '環境1', imageId: baseImage.image.id });

  await page.evaluate(() => {
    window.__ccImageLogs = [];
    window.__ccProvisionLogs = [];
    window.__ccSetupLogs = [];
    window.__ccAppLogs = [];
    window.__ccResets = [];
    window.cc.onLog((line) => {
      if (line.stream === 'image') window.__ccImageLogs.push(line.text);
  ...
  check('a config key that no longer exists is rejected', unknownKey.ok === false, errorText(unknownKey));
  const legacyTag = await call(page, 'configSave', [{ imageTag: 'x' }]);
  check('the removed imageTag setting is rejected', legacyTag.ok === false, errorText(legacyTag));
  check(
    'a fresh config seeds no environment at all',
    boot.config.environments.length === 0 && boot.config.defaultEnvironmentId === null,
  );
  check(
    'the first saved environment becomes the default',
  ...
  check(
    ...
    refused.ok === false && /取り出せなかった|could not be exported/u.test(errorText(refused)),
    refused.ok ? 'deleted!' : errorText(refused),
  );
  snapshot = await ok(page, 'snapshot');
  ...
  const partialPath = refused.ok ? '' : (/the partial export is at (.+)$/u.exec(errorText(refused))?.[1] ?? '');
  check(
  ...
  check(
    ...
    cancelled.ok === false,
    cancelled.ok ? 'deleted!' : errorText(cancelled),
  );
  ...
  const releaseTwo = await session.registerImage(session.fixture.releaseTwo.id);
  snapshot = await ok(page, 'snapshot');
  check(
    'registering a new release does not flag the existing tasks',
    taskById(snapshot, alpha.id)?.imageStale === false && taskById(snapshot, beta.id)?.imageStale === false,
  );
  const imageLogs = await page.evaluate(() => window.__ccImageLogs ?? []);
  check('download progress was streamed to the log pane', imageLogs.length > 1, `${imageLogs.length} lines`);
  const envOne = snapshot.config.environments.find((environment) => environment.id === defaultEnvironmentId);
  const envOneDraft = {
    id: defaultEnvironmentId,
    name: envOne.name,
    envText: envOne.envText,
    setupScript: envOne.setupScript,
  };
  await ok(page, 'environmentUpsert', [{ ...envOneDraft, imageId: releaseTwo.image.id }]);
  snapshot = await ok(page, 'snapshot');
  check(
    'switching the environment image flags every task on that environment',
    taskById(snapshot, alpha.id)?.imageStale === true && taskById(snapshot, beta.id)?.imageStale === true,
  );
  check('an image switch alone is not an environment change', taskById(snapshot, alpha.id)?.environmentStale === false);
  ...
  check(
    ...
    recreated?.imageStale === false && recreated?.container.registeredImageId === releaseTwo.image.id,
    String(recreated?.container.registeredImageId),
  );
  check(
  ...
  check(
    ...
    (await sh(page, alpha.id, 'cat /opt/cc/e2e-release-two')).stdout.trim() === 'release two',
  );
  ...
  snapshot = await ok(page, 'snapshot');
  check('back on release one, the untouched task is current again', taskById(snapshot, beta.id)?.imageStale === false);
  check('and the recreated one is now the stale one', taskById(snapshot, alpha.id)?.imageStale === true);
  await ok(page, 'taskRecreate', [alpha.id]);
  check('recreating again lands on release one', taskById(await ok(page, 'snapshot'), alpha.id)?.imageStale === false);
  ...
  const savedEnv = await ok(page, 'environmentUpsert', [
    { id: 'e2e-env', name: '  E2E   env ', imageId: baseImage.image.id, envText, setupScript },
  ]);
  ...
  await ok(page, 'environmentUpsert', [
    { id: 'e2e-env', name: 'E2E env', imageId: baseImage.image.id, envText: 'CC_E2E_MARKER=edited', setupScript },
  ]);
  snapshot = await ok(page, 'snapshot');
  ...
  await ok(page, 'environmentUpsert', [
    {
      ...
      envText: '# only a comment was added\nCC_E2E_MARKER=edited',
      setupScript,
    },
  ]);
  check(
  ...
  check('an archived environment cannot be picked for a new task', onArchived.ok === false, errorText(onArchived));
  ...
  check('an environment a task uses cannot be deleted', deleteInUse.ok === false, errorText(deleteInUse));
  ...
  const navLabels = () =>
    page.evaluate(() =>
      [...document.querySelectorAll('.sidebar-nav button')].map((button) => button.textContent?.trim() ?? ''),
    );
  const englishNav = await navLabels();
  check(
    'UI switched to English',
    englishNav[0]?.includes('Images') === true && englishNav.some((label) => label.includes('Profiles')),
    englishNav.join(' / '),
  );
  await ok(page, 'setLanguage', ['ja']);
  await page.waitForTimeout(700);
  const japaneseNav = await navLabels();
  check(
    'UI switched back to Japanese',
    japaneseNav[0]?.includes('イメージ') === true && japaneseNav.some((label) => label.includes('プロファイル')),
    japaneseNav.join(' / '),
  );
