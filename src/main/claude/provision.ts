...
async function isManagedTmuxConf(path: string): Promise<boolean> {
  try {
    const head = (await readFileRaw(path)).subarray(0, 200).toString('utf8');
    return head.startsWith(TMUX_MANAGED_MARKER) || head.startsWith('# Generated by cc-container-desktop');
  } catch {
    return false;
  }
}
...
async function provisionTmux(): Promise<void> {
  ...
  if (!ours) {
    logWarn(
      'provision',
      `~/.tmux.conf は編集済みなので触りません。設定は ${TMUX_MANAGED_CONF} にあります / left your ~/.tmux.conf alone; the managed settings are in ${TMUX_MANAGED_CONF}`,
    );
    return;
  }
