...
const TMUX_LEGACY_SOURCE = `# Generated by cc-container-desktop (only when no ~/.tmux.conf exists).
...
async function isManagedTmuxConf(path: string): Promise<boolean> {
  let current: string;
  try {
    current = (await readFileRaw(path)).toString('utf8');
  } catch {
    return false;
  }
  if (current.startsWith(TMUX_MANAGED_MARKER)) return true;

  return current === TMUX_LEGACY_SOURCE;
}

async function provisionTmux(): Promise<void> {
  await execChecked(['mkdir', '-p', TMUX_MANAGED_DIR], { workdir: '/' });
  await writeFileText(TMUX_MANAGED_CONF, TMUX_MANAGED_SOURCE, 0o644);

  const ours = !(await fileExists(TMUX_CONF)) || (await isManagedTmuxConf(TMUX_CONF));
  if (!ours) {
    logWarn(
      'provision',
  ...
  await writeFileText(TMUX_CONF, TMUX_STUB_SOURCE, 0o644);

  const reloaded = await execCapture(['tmux', 'source-file', '-q', TMUX_CONF]);
