...
# The container is the sandbox and everything in it runs as root; Claude Code
# skips permission prompts under uid 0 only when it is told that much.
export IS_SANDBOX=1
...
export async function provisionTask(task: Task): Promise<ProvisionOutcome> {
  ...
  await execChecked(ref, ['mkdir', '-p', CLAUDE_DIR, CONTAINER_WORKSPACE, CONTAINER_SCRIPT_DIR], { workdir: '/' });
